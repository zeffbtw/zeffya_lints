...
class ClassMemberOrderFunctions {
  ...
  static List<String> getOrderViolations(List<ClassMemberType> types) {
    ...
    for (int i = 1; i < types.length; i++) {
      if (types[i].index < types[i - 1].index) {
        violations.add('| <${types[i].name}> must be before <${types[i - 1].name}> |');
        break;
      }
    }
    return violations;
  ...
  static ClassMemberType computeClassMemberType(
    ...
    if (member is ConstructorDeclaration) {
      ...
      if (member.isStatic && member.fields.isConst) return ClassMemberType.staticConst;
      if (member.isStatic && member.fields.isFinal) return ClassMemberType.staticFinal;
      if (member.isStatic) return ClassMemberType.static;
      if (member.fields.isConst) return ClassMemberType.constant;
      if (member.fields.isFinal && member.fields.isLate) return ClassMemberType.lateFinal;
      if (member.fields.isFinal) return ClassMemberType.finalVar;
      if (member.fields.variables
          .any((v) => v.declaredElement?.type.nullabilitySuffix == NullabilitySuffix.question)) {
      ...
      if (member.name.lexeme.startsWith('_')) return ClassMemberType.privateMethod;
